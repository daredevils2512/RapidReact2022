...
public class findRange extends CommandBase {
  ...
  public void execute() {
    ...
    m_angleToGoalDegrees = Constants.LIMELIGHT_MOUNT_ANGLE.add(m_ty);
    m_angleToGoalRadians = m_angleToGoalDegrees * (Math.PI / 180.0);
    m_currentDistance = (Constants.GOAL_HEIGHT.subract(Constants.LIMELIGHT_LENS_HEIGHT)) / Math.tan(m_angleToGoalRadians);
    m_distanceVariation = Constants.DESIRED_DISTANCE.subtract(m_currentDistance);
